...
var fixPrinter = function() {
    ...
    if ($("not_printing").selected===false && $("yes_printing").selected === false ||
        ...
        $("not_recognised").selected===false && $("yes_recognised").selected === false){
            alert("You need to answer all three questions.");
            stringOutput = "Please answer all of the questions to continue";
        } else if (boolPrinting === true) {
      if (boolRedLight === true) {
          if (boolRecognised === true) {
              stringOutput = stringMessage2 + "\n" + stringMessage3 + "\n" + stringMessage4; //all values are true
          } else {
             stringOutput = stringMessage4 + "\n" + stringMessage5; //not recognized will be false
          }
      } else if (boolRecognised === true) {
          stringOutput = stringMessage1 + "\n" + stringMessage2 + "\n" + stringMessage3; //printer does not print and printer is unrecognized 
        } else {
          stringOutput = stringMessage5; //printer does not print 
        } 
    } else if (boolRedLight === true) {
          if (boolRecognised === true) {
              stringOutput = stringMessage3 + "\n" + stringMessage4; //red light is flashing and printer is unrecognised 
          } else {
             stringOutput = stringMessage4; //red light is flashing
          }
      } else if (boolRecognised === true) {
          stringOutput = stringMessage3; //printer is unrecognized 
        } else {
          stringOutput = stringMessage6; //everything is working on the printer!
        } 
