...
var passwordCheckOld = function () 
{
    ...
    stringPassword = $("password").value; //DOM Input 
    
    stringOutput = "";
 
   if (stringPassword.length<8) //password cannot be less than 8 characters 
   {
    stringOutput="Your password must be greater than 8 characters"; //returns message when the Password is less than 8 characters. 
   }
   else 
   {
  for (var a=0; a < arrayBadPasswords.length; a++) //initializing the loop
  {
    if (stringPassword === arrayBadPasswords[a]) //checks to see whether the password matches any of the ones in the array.
    {
        alert("Please choose another Password"); //returns alert when password matches any of the elements in the array.
    }
    else
    {
      stringOutput = "Your Password has been successfully created."; //When Password meets all the criteria, it is successfully created. 
    }
  }
   }
$("output").value = stringOutput; //push the value from the DOM to the HTML 
};
